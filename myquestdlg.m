function ButtonName=myquestdlg(Question,Title,Btn1,Btn2,Btn3,Default, varargin)
%MyQUESTDLG Question dialog box.
%Copy of Matlabs code, + passes extra arguments to dialog(...) so that
%any figure property can be set
...
while j <= length(varargin)
    ...
    if isfield(varargin{j},'FontSize')
        figargs = {figargs{:} 'DefaultUIControlFontSize' varargin{j}.FontSize};
    end
    if isfield(varargin{j},'FontName')
        figargs = {figargs{:} 'DefaultUIControlFontName' varargin{j}.FontName};
    end    
    else
        figargs = {figargs{:} varargin{j}};
    end
    j = j+1;
end
