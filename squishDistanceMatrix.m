...
while j <= length(varargin)
    ...
    elseif strncmpi(varargin{j},'laps',3)
        j = j+1;
        laps = varargin{j};
    elseif strncmpi(varargin{j},'order',3)
        j = j+1;
        tryorder = varargin{j};
...
if ~isempty(tryorder)
    squishedMatrix = remapDistanceMatrix(originalDistanceMatrix, tryorder);
    score = squeezeScore(squishedMatrix,dd);
    return;
end
if markwait
...
%        scores = zeros(1,n);
        ...
        while stop == 0
            ...
            while j < size(swaps,1)
                ...
                if d < bestscore && d > 0 %when score improves, check all permutations again
                    ip = ip+1;
                    scores(ip) = squeezeScore(remapedDM,dd);
                    p(ip,:) = neworder;
                    oldorder = neworder;
