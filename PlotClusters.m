...
%       PlotAllCell(DATA, type); %surely don't need this
    ...
    elseif strcmp(fcn,'probes')
       if strmatch(type,{'spoolall' 'spoolcells' 'allspks' 'allcellspks' 'spooleverything' 'spooleverycell' 'selectspks'})
    ...
    elseif strncmp(fcn,'allcluster',8)
        if strcmp(type,'markmanual')
            DATA.plot.markmanual = ~DATA.plot.markmanual;
            set(a,'Checked',onoff{DATA.plot.markmanual+1});
            DATA = PlotAllClusters(DATA,[]);
        elseif strcmp(type,'markauto')
            DATA.plot.markauto = ~DATA.plot.markauto;
            set(a,'Checked',onoff{DATA.plot.markauto+1});
            DATA = PlotAllClusters(DATA,[]);
        elseif strcmp(type,'markcopy') %copied from anohter expt
            DATA.plot.markcopy = ~DATA.plot.markcopy;
            set(a,'Checked',onoff{DATA.plot.markcopy+1});
            DATA = PlotAllClusters(DATA,[]);
        end
...
function [true, cellid] = isacell(DATA, row, p)
    ...
    function OptionMenu(a, b, fcn)
        ...
        elseif strcmp(fcn,'xcorrcells')
            ...
            toc
        elseif strmatch(fcn,{'xcorrallcells' 'xcorrallprobes' 'recalcxcorrcell' 'recalcxcorrall'})
...
function plots = PlotClusterXY(DATA, C, varargin)
 ...
 function PlotExptCounts(DATA, e, p, varargin)
     ...
     if strncmpi(DATA.plotexpttype,'trialcounts',10)
         PlotExpt(Expt,'seqt','rcnmin',10);
     elseif strncmpi(DATA.plotexpttype,'none',4)
     else
         PlotExpt(Expt,'shown','rcnmin',10);
     end
...
if ismember(type, [1 3]) % 3 = hit cell image - set cell#
    ...
    nc = Counts(C.clst);
    nstr = sprintf(' %d',nc);
    fprintf('P%d E%.0f cut on %s:%s spks\n',p,C.exptno,datestr(C.ctime),nstr);
    if type == 3
       ...
       if cells > 0
           ...
           if lastcell ~= DATA.currentcell
               if DATA.plotallxy   %hit cell image plot
                   PlotAllCellXY(DATA);
               end
               if DATA.plotspks
                   PlotAllCell(DATA,'allspkswithmean');
...
DrawBox(DATA.selectexpts,DATA.proberange)
end
...
function [f, isnew] = SetFigure(DATA, tag, varargin)
    ...
    set(f,'UserData', DATA.toplevel);
        ...
        elseif strfind(tag,'ClustersAll');
            ...
            uimenu(hm,'Label','Autol Cut','Callback',{@PlotMenu, 'allcluster' 'markauto'});
            uimenu(hm,'Label','Manual Cut','Callback',{@PlotMenu, 'allcluster' 'markcopy'});
            set(f,'UserData', DATA.toplevel);
...
%C.xy should no longer be hcange by plotclusters, so don't save it out        
...
%thing is just to make changes separatley, not copy one to anaother
        if C.cluster > 1
            ...
            Clusters{p}.next{n}.angle = C.next{n}.angle;
            Clusters{p}.next{n}.savetime(1) = now;
    ...
    for j = 1:length(expts)
        ...
        if strmatch(type,{'allprobespks' 'alltemplatespks'})
            ...
            if a > 0 || sum(strcmp(type,{'allprobespks' 'alltemplatespks'}))
                h = QuickSpikes(DATA, AllSpikes{eid,p},C,'showmean');
                axdata.lines = h;
...
function S= CheckAllSpikes(DATA, e,p)
...
function DATA = PlotAllProbe(DATA, type)
    ...
    %'exptspks', 'allspks' plot all probes for one expt
     if strmatch(type,{'allspks' 'allcellspks' 'spoolall' 'exptspks'})
         ...
         pid = 1:np;
     elseif strmatch(type,{ 'allprobespks' })
         np =length(Clusters);
         expts = 1:np;
    ...
    [nr,nc] = Nsubplots(np);
     ...
     if strmatch(type,{'allspks' 'allcellspks' 'allprobespks' 'selectspks'})
    ...
    axdata.toplevel = DATA.toplevel;
     ...
     for j = 1:np
        mysubplot(nr,nc,j,'tight','width',0.95);
        hold off; 
        p = pid(j);
        if strmatch(type,{'allspks' 'allcellspks' 'selectspks' 'exptspks' 'allprobespks'})
            ...
            h = QuickSpikes(DATA, AllSpikes{eid,pid(j)},C,'showmean');
            axdata.lines = h;
            ...
            if a > 0 || c > 0
                if (a > 0)
                th = title(sprintf('Cell %s E%.0fP%d 1D%.1f 2D%.1f %s',sprintf('%d ',b),C.exptno,pid(j),C.mahal(4),C.mahal(1),dstr));
                set(th,'color','b','fontweight','bold');
                else
                title(sprintf('E%.0fP%d 1D%.1f 2D%.1f %s',C.exptno,pid(j),C.mahal(4),C.mahal(1),dstr));
                end
...
function DATA = InitInterface(DATA)
    ...
    uicontrol(gcf,'style','pop','string','quality|times|xcorr|ptscatter|mahaldip|bestspace|bestdip|bmcmahal|cov(shape)|xc(shape)', ...
        ...
        'units', 'norm', 'position',bp,'value',1);
    bp(1) = bp(1)+bp(3)+hspace;
    uicontrol(gcf,'style','pop','string','mahal|dips|mahaln|mahal1-n|mahal+var|man-auto|drop-mahal|Fit-mahal|muamp-spkvar|xcorr|probexcorr|probeco|spkshapecorr|spkshape|spksize|Evec|BuildTimes|PcGms|Exclusions|Tagged|TriggerFind|CandidateCells|SpkRate|EventRate', ...
    ...
    uicontrol(gcf,'style','checkbox','string','PlotExpt', ...
        ...
        'units', 'norm', 'position',bp,'value',plotexpt);
    bp(1) = hspace;
    bp(3) = 0.2;
    bp(2) = bp(2)+ 1./nr;
    ...
    uimenu(sm,'Label','All XY This Probe','Callback',{@PlotMenu, 'probes', 'AllprobeXY'});
    ...
    uimenu(sm,'Label','Recalculate All','Callback',{@OptionMenu, 'recalcxcorrall'});
   ...
   if strcmp(type,'ifnew')     && length(reloaded) && DATA.spikesloaded
    ...
    if loadexpts && length(AllExpts)
        ...
        for j = 1:size(Expts,1)
            ...
            DATA.exped(j) = mean(GetEval(Expts{j,1},'ed'));
        end
...
h = plot(square(1,:),square(2,:),'k-','buttondownfcn',{@HitImage, 1},'linewidth',2);
if length(varargin)
    set(h,varargin{:});
end
...
function DATA = SpoolCurrentSpikes(mn,b)
    ...
    if isempty(AllSpikes)
        ...
        drawnow;
        SetTrialList(DATA,Clusters{DATA.currentpoint(1)}{DATA.currentpoint(2)});
        SpoolSpikes(DATA, DATA.currentpoint);
    end
...
if strmatch(ks.Key,'delete') 
    ...
    elseif fcn == 3  %delete current location from CellList
        ...
        DATA = DeleteCell(DATA, DATA.currentpoint(1),DATA.currentpoint(2),DATA.cellcluster);
        PlotCellList(DATA,'showfig');
        set(DATA.toplevel,'UserData',DATA);
    end
...
function h = QuickSpikes(DATA, pos, varargin)
    ...
    if isfield(Spks,'maxv')
        Spks.values = double(Spks.values) .* Spks.maxv./16000;
    end
    ...
    if showmean
        ...
        for j = 2:length(h)
            if h(j) > 0 & ishandle(h)
                ...
                color = 1-color;
                if j > 2
                    plot(C.next{j-2}.MeanSpike.ms(pos(2),:),'-','color',color,'linewidth',2);
                else
                    plot(C.MeanSpike.ms(pos(2),:),'-','color',color,'linewidth',2);
                end
            end
        end
    end
...
if isfield(Spks,'maxv') && strcmp(class(Spks(1).values),'int16')
    for j = 1:length(Spks)
    Spks(j).values = double(Spks(j).values) .* Spks(j).maxv./16000;
    end
end
if fixy
    yl = get(gca,'ylim');
