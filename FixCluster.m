...
if ~isfield(C,'minenergy')
    nerr = nerr+1;
    errs{nerr} = 'nominenergy';
    C.minenergy = 0;
end
if ~isfield(C,'manual')
    nerr = nerr+1;
...
C = AddField(C,{'isSU' 'usestdtemplates'},0);
...
if isfield(C,'mahal') && length(C.mahal) < 4
    C.mahal(4) = NaN;
end
if ~isfield(C,'next')
    C.next = {};
...
if ~isfield(C,'probe') && isfield(DATA,'probe')
    C.probe = DATA.probe;
end
