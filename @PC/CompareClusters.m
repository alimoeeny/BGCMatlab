...
if isfield(Ca,'clst') && isfield(Cb,'clst')
    ...
    if length(Ca.times) == length(Ca.clst) %ClusterDetails
        ...
        if strcmp(plottype,'all')
            ...
            hold off;
            PC.QuickSpikes(DATA, AllSpikes{e,p}, Ca,'oneprobe','order',Caorder,'oneprobe');
            PC.PlotClusterXY(DATA,Ca,'cluster',useclusters(1));
            nev = length(Ca.clst);
            if ~isempty(useclusters)
            ...
            subplot(2,2,3);
            PC.QuickSpikes(DATA, Spikes, Cb,'order', Cborder,'oneprobe');
            nev = length(Cb.clst);
            if length(useclusters) > 1
