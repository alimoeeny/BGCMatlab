...
if sum(strcmp(PlotData.plottype,{'efficacies' 'cellefficacies'}))
    ...
    %matches == 5 means cell with match
    %matches ==4 means cell without match
...
if cls(2) == 0
    ...
    fprintf('E%dP%dCl%d Cell%d(%d)->Auto%d(%d) %.2f A->C %.2f(%s)\n',e,p,cls(1),cellno,nc(1),cls(2),nc(2),A.efficacy(id,1,cls(2)),A.efficacy(id,2,cls(2)),autoC.autocutmode);
end
...
function LabelMeanProbe(C)
nprobes = 1;

sd = std(C.MeanSpike.ms,[],2);
[a,b] = sort(sd,'descend');
if nprobes == 0
    nprobes = sum(b > bb(1)/2);
end
for j = 1:nprobes
[c,d] = max(abs(C.MeanSpike.ms(b(j),:)));
text(d,C.MeanSpike.ms(b(j),d),sprintf('P%d',b(j)));
end