...
if isnew
    ...
    if ~isempty(X) && size(X,2) > 2 %!! size can be 0x3
        a = CellToStruct(X(:,3));
        nr = 1+sum([a.on]);
        id = find([a.on]);
    ...
    for j = 1:size(X,1)
        if X{j,3}.on
            ...
            nbutton = nbutton+1;  %in case the off buttons are real
        end
    end
%if no buttons found, don't popup;    
    if nbutton == 0 && isempty(tag)
        if sum(good) < size(X,1)
            X = X(find(good),:);
