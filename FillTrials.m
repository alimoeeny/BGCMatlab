...
elseif strcmp(code,'cR')
    if isfield(Expt.Trials,'ic') && isfield(Expt.Trials,'co')
        idx = find([Expt.Trials.Trial]);  
        for j = 1:length(idx)
            T = Expt.Trials(idx(j));
            Expt.Trials(idx(j)).cR = T.co - T.ic;
        end
        vals = unique([Expt.Trials.(code)]);
        id = find(diff(vals) < 1e-6)
        for j = 1:length(id)
            idx = find([Expt.Trials.(code)] == vals(id(j)));
            for k = 1:length(idx)
                Expt.Trials(idx(k)).(code) = vals(id(j)+1);
            end
        end
    end
elseif strcmp(code,'cL')
    if isfield(Expt.Trials,'ic') && isfield(Expt.Trials,'co')
        idx = find([Expt.Trials.Trial]);  
        for j = 1:length(idx)
            T = Expt.Trials(idx(j));
            Expt.Trials(idx(j)).cL = T.co + T.ic;
        end
        vals = unique([Expt.Trials.(code)]);
        id = find(diff(vals) < 1e-6)
        for j = 1:length(id)
