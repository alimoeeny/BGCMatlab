...
%
%GetFigure(0) prints a list of tags associated with current figures
%See also SetFigPos
doclear = 0;
j = 1;
fk = 1;
...
if tag == 0 %list current figures and tags
    figlist = findobj(get(0,'children'),'flat','Type','Figure',varfind{:});
    [~, figorder] = sort(double(figlist));
    figlist = figlist(figorder);
    for j = 1:length(figlist);
        fprintf('%.0f:%s\n',double(figlist(j)),get(figlist(j),'Tag'));
    end
    return;
end
new = 0;
