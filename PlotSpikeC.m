...
%These blocks are NOT included in the FullV
        ...
        for j = 1:length(id)
            ...
            fprintf('%s\n',msg);
            res = AddError(res, '-prog','BuildFullV', msg);
        ...
        if sumt > 10
            res.error = 1;
            AddError(res, '%.1f Seconds of missing channels - Aborting write\n',sumt);
            return;
        end
...
%    [a,b] = sort(chspk);
    ...
    for k = 1:length(fid)
        ...
        if isstruct(C)
            ...
            if ~exist(name)
                res = AddError(res,'!!!\nMissing Spkblk File %s!! Probe index out of date or files were deleted\n!!!',name);
                res.error = 1;
                return;
            end
            F = load(name);
...
%        V = V - repmat(sumv,size(V,1),1);
    ...
    if checkmains %2 = build average but don't remove. Checks that mean removal worked
...
%        smv(j,:) = smooth(V(j,:),80);
    ...
    if makeV
        ...
        res.progname = 'BuildFullV';
        try
            chk = fullv.Check(res, Expt);
            res = AddError(res,chk);
            res.missingtrials = chk.missingtrials;
            for j = 1:length(chk.missingtrials)
                T = Expt.Trials(chk.missingtrials(j));
                t = [T.Start(1) T.End(end)]./10000;
                pid = find([C.start] < t(1) & [C.end] > t(2));
                missing = setdiff(res.chspk,[C(pid).probe]);
                res = AddError(res,'-show','Trial %d(id%d) missing probes %s',T.Trial,T.id,sprintf(' %d',missing));
            end
        catch ME
            CheckExceptions(ME);
        end
...
%end
    ...
    if checkblocks
        if ~isempty(Expt)
            expts = GetExptNumber(Expt);
        else
            expts = unique([C.suffix]);
        end
        for j = 1:length(expts)
            pid = find([C.suffix] == expts(j));
            [chspk, lens, starts, offsets, f, X] = CountProbeChans(C(pid), spk);
            if isfield(X,'errs')
                res.errmsg = {res.errmsg{:}, X.errs{:}};
            end
            if isfield(X,'errdata')
...
for j = unique(chspk)
    ...
    if ~isempty(id)
        badid = chid(order(id(1)));
        nextid = chid(order(id(1))+1);
        X.probe = chspk(j);
        X.suffix = C(badid).suffix;
        X = AddError(X,'CH%dE%d %d blocks end(n) > start(n+1). %d by > %.0fms  (Starts %s) (Ends %s)\n',...
            chspk(j),X.suffix,length(bzid),length(id), MAXOVERLAP,...
            sprintf('%.4f ',starts(chid(order(id+1)))),sprintf('%.4f ',ends(chid(order(id)))));
        X.errdata(end).bwtime = starts(chid(order(id+1)));
        
    elseif ~isempty(bzid)
        badid = chid(order(bzid(1)));
        X.probe = chspk(j);
        X.suffix = C(badid).suffix;
        X = AddError(X,'CH%dE%d %d blocks end(n) > start(n+1). But all < 2ms  (Starts %s) (Ends %s)\n',chspk(j),X.suffix,length(bzid), ...
            sprintf('%.4f ',starts(chid(order(bzid+1)))),sprintf('%.4f ',ends(chid(order(bzid)))));
        X.errdata(end).bwtime = starts(chid(order(bzid+1)));
    end
    for k = 1:length(id)
        a = chid(order(id(k)));
        b = chid(order(id(k)+1));
