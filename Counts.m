...
elseif iscell(x)
    ...
    if max(lens) > 1
        ...
        vals = unique(cat(1,x{:}));
%used to do this. but bad to make output look just like the input was
%numeric. count returned now has special meaning for cell arrays
...
if ~isempty(count) && ~iscell(x)
    count = reshape(count,size(vals));
end