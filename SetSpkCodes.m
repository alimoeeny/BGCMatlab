...
while j <= length(varargin)
    if strncmp(varargin{j},'exptid',6)
        j = j+1;
        nexp = varargin{j};
    end
    j = j+1;
...
if max(expspks) > length(Cx)
    fprintf('Truncating Spike Values %d -> %d\n',max(expspks),length(Cx));
    expspks = expspks(expspks<length(Cx));
end
%
    ...
    while ~isempty(C) & isfield(C,'x')
        if C.params(1) ~= DATA.plot.clusterX || C.params(2) ~= DATA.plot.clusterY
            if isfield(Spikes,'cache') %will need spike values again
                DATA = cmb.LoadSpikes(DATA, nexp(1), probe,'force');
                Spikes = DATA.AllData.Spikes;
            end
        end
...
%        dprimes(j) = abs(mean(d(nid))-mean(d(id)))./std(d(nid));
    ...
    title(sprintf('%s dprime %.2f(%.2f) %.1fHz:%d/%dspks',DATA.explabels{nexp},dprime(cl),bii,rate,length(id),length(cspks))); 
        end
    else
        if isfield(DATA,'explabels') && length(DATA.explabels) >= nexp(1) && cl == DATA.currentcluster
    title(sprintf('%s dprime %.2f %d/%dspks',DATA.explabels{nexp(1)},dprime(cl),length(id),length(cspks))); 
        end
    end
    end
    DATA.cluster{cl,p}.nspk = nspk;
end %for cl = cllist
    DATA = cmb.SpkCache(DATA,nexp(1),DATA.probe,'set');
