                ...
                elseif regexp(d(j).name,'ClusterTimes.mat') %% d(j).bytes < 525000000 & np < 2
                    ...
                    for k = 1:length(a.Clusters)
                        if ~isempty(a.Clusters{k})
                            a.Clusters{k}.auto = SpkTimes(nspkt).auto;
                            mint = min([mint min(a.Clusters{k}.times)]);
                            maxt = max([mint max(a.Clusters{k}.times)]);
                        end
                        if quickload == 0 && ~isfield(a.Clusters{k},'xy') && isfield(SpkTimes(1).Clusters{k},'xy')
...
%sort by time of first spike keep track of total # spikes
  ...
  vnames = {'Ch30' 'Ch31' spkch framechname stimch};
    vlabels = {'Text' 'Events' 'Spikes' 'Frames' 'Stim ON/OFF'};
    ...
    Text = Ch30;
    if ignoreSpikeO == NaN && length(probes) > 1   && Oprobe > 1
        ...
        if nspkt
            ...
            for j = expts
                ...
                aid = aid(1);
                    ...
                    for c = 1:length(SpkTimes(aid).Clusters);
                        if c > length(Clusters) || isempty(Clusters{c})
                            Clusters{c} = SpkTimes(aid).Clusters{c};
                        end
                    end
                elseif length(aid)
            ...
            for j = 1:length(Clusters)
                ...
                if isfield(Clusters{j},'times')
                    probes(j).probe = j;
                    if diff(size(Clusters{j}.times) > 1)
...
%storage is off, resetting the last stored trials
 ...
 %           Trials.scoretime(trial-1) = aText.times(j);
        elseif aText.codes(j,1) == BADFIX
            ...
            if aText.times(j) > Trials.End(trial)
...
%                end
    ...
    if length(Trials.End) >= trial && t > Trials.End(trial) & instim & trial <= length(Trials.Start)
