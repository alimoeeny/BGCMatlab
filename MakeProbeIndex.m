function [probes, errs, proberrs]  = MakeProbeIndex(dname, varargin)
...
%
%     MakeProbeIndex(path,'recalc') rebuilds list
%     MakeProbeIndex(path,'update') adds in new spkblk files
...
elseif exist(outfile,'file') && ~recalc && isempty(probes)
    ...
    load(outfile);
    if exist('errs','var')
        CheckExceptions(errs);
    end
    if checkprobes
        proberrs = CheckProbes(probes, verbose);        
    end
    return;
end
...
if isempty(d) || isempty(filesuff)
    if ispc && ~isempty(Spike2.path('sonview')) && ~isempty(dosuffs) %Can call Spike2
        yn = gui.Dlg('No Spkblk files. Call Spike2','MakeProbeIndex',{'Yes', 'No'});
        if strcmp(yn,'Yes');
            Spike2.MakeSpkBlk(dname,'expts',dosuffs);
        else
            fprintf('Cannot Build FullV without spkblk files\n');
        end
    else
...
for m = 1:length(ids)
    ...
    if length(sid) & ~ismember(filesuff(idlist(m)),oldsuff) & ismember(filesuff(idlist(m)),dosuffs);
        ...
        try
            if verbose > 2
                PrintMsg(logfid,'Loading %s nc is %d at %.1f sec',fname,nc,mytoc(ts));
            end
            clear a; %so that if load fails, don't reuse old a....
            a = load(fname);
        catch ME
