function [CD, Clusters] = CheckExptClusters(dname, varargin)
% [CD, Clusters] = CheckExptClusters(D, varargin) checks for misisng trials in Clusters
% if D is a folder, checks all Clusters and Expts.
%
%returns clusterdetails and clusters
...
%disk
%
%%[CellDetails, Clusters] = CheckExptClusters(...  also returns Clusters
%%data structs
...
while j <= length(varargin)
    ...
    elseif strncmpi(varargin{j},'nocells',5)
        needcells = 0;
...
checktime = now;
outname= [dname '/ClusterCheck.mat'];
LastCheck = my.load(outname,'safe');
...
elseif isdir(dname)
    if isempty(Expts)
        Expts = ReadExptDir(dname,readexptargs{:});
    end
    exptid = GetExptNumber(Expts);
    if ~isempty(eid)
        id = find(ismember(exptid,eid));
    ...
    if ~isempty(Expts)
        eid = GetExptNumber(Expts);
    end
%Check for cells before loading Cluster Data
    cellname = [dname '/CellList.mat'];
    if exist(cellname)
        cells = load(cellname);
        CD = cells.CellDetails;
    elseif needcells
        fprintf('No CellList file in %s. Use ''nocells'' flag to force checking\n',dname);
    end
    if isfield(LastCheck,'CD') && isfield(LastCheck.CD,'checktime')
        X = LastCheck.CD;
        for j = 1:length(eid)
            e = eid(j);
            a = dir(sprintf('%s/Expt%dClusterTimes.mat',dname,e));
            if isempty(a) %Don' have clusters for this expt
                cprintf('red','No ClusterTimes for Rxpt %d\n',e);
                need(e) = 0;
            elseif length(X.checktime) >= e && X.checktime(e) > a.datenum
                need(e) = 0;
            else
                need(e) = 1;
            end
        end
        if sum(need(eid)) == 0 && ~force
            fprintf('No changes in Clusters for %s since last check\n',dname);
            ShowErrors(X);
            CD = CopyFields(CD, X); %combine with CD in CellList
            return;
        end
    end

    [Clusters, FullVData, cdetails] = LoadCluster(dname, eid,'array','condense');
    for j = 1:length(cdetails)
        CD = AddError(CD,cdetails{j},'-prog','LoadCluster');
    end
...
for j = 1:length(Expts);
   ...
   if ~isempty(Expts{j}) && ~isempty(cid)
       if length(cid) > 1
           fprintf('Expt %d Matches %s\n',exptid(j),sprintf(' %d',cid));
           cid = cid(1);
       end
       ...
       if length(cdetails) >= cid && isfield(cdetails{cid},'loadname')
           clusterfile = cdetails{cid(1)}.loadname;
       end
       clustermodified = 0;
       ...
       for c = 1:length(C)
           ...
           missing = sum(n==0);
           CD.nspks(exptid(j),c) = sum(n);
           CD.nevents(exptid(j),c) = C{c}.nspks;
           if isfield(C{c},'clst')
               CD.nclusters(exptid(j),c) = length(unique(C{c}.clst));
           end
           if missing 
               mid = ids(n==0);
           ...
           if isfield(C{c},'autofits')
               fits = C{c}.autofits;
               for k = 1:length(fits)
                   a = length(fits{k}.clst);
                   if a ~= C{c}.nspks
                       CD = AddError(CD,'E%dP%d %d spks but autofit has %d',exptid(j),p,C{c}.nspks,a);
                   elseif verbose >1
                       fprintf('E%dP%d %d spks matches clst in fits %d\n',exptid(j),p,C{c}.nspks,k);
                   end
               end
           end
       end %end for length C
...
%duplicates
   ...
   else %Just check Cluster Struct - cannot compare with Expts/CellList
       
       if ~isempty(Clusters{j})
           if isfield(Clusters{j}{1},'MeanSpike')
               nprobes = size(Clusters{j}{1}.MeanSpike.ms,1);
           elseif isfield(FullVData{j},'ArrayConfig')
               nprobes = length(FullVData{j}.ArrayConfig.id);
           end
           if nprobes > 1
               CD.gotprobes(j) = length(Clusters{j});
               if length(Clusters{j}) < nprobes
                   CD = AddError(CD, '-show', 'E%d only %d/%d Clusters',eid(j),length(Clusters{j}),nprobes);
               end
           else
               CD = AddError(CD, '-show', 'E%d Cannot determine NProbes',eid(j));
           end
       else
           CD.gotprobes(j) = 0;
       end
...
for j = 1:length(CD.errs)
    estr = '';
    if CD.errdata(j).time > CD.progchecktime-0.1
