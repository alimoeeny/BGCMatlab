...
if BuildV
    ...
    if length(gotexpts) == 1  && online  %off for now
        e = gotexpts;
        DATA.tfirst = (Expts{e}.Trials(1).Start - preperiod)/10000;
        if submains
...
%    parfor (ex = 1:length(gotexpts))
    for ex = 1:length(gotexpts)
        ...
        onetrig = [];
        id = find(DATA.idx.expt == e);
        tfirst = (Expts{e}.Trials(1).Start - preperiod)/10000;
...
%                Arrays{e} = Array;
            ...
            FullV.savetime = now;
            fprintf('Writing P%d(%d) %s (%s) (took %.2f at %s)\n',p,FullV.chspk,outfile,outvarname,mytoc(ts),datestr(now));
...
if refcut
    ...
    allvargs = {'matchcounts' 'nowatch' 'nocheck'};
        ...
        for p = probes;
            ...
            if isfield(Clusters{p},'shape')
                if isfield(Clusters{p},'marked') && Clusters{p}.marked == 2
                    AllVPcs(outfile,'tchan',p,'GridData','savespikes','reapply',Clusters{p},'refine',allvargs{:});
                else
                    AllVPcs(outfile,'tchan',p,'GridData','savespikes','reapply',Clusters{p},allvargs{:});
                end
            else
