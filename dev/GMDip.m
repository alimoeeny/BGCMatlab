...
if usecritonly == 0
    ...
    d(2) = gmdprime(Gs{2});
        ...
        fprintf('GMDip 2 fit (%.3f, %.3f sd %.3f) failed-%s\n',S.mu(1,1),S.mu(2,1),sigma,lasterr);
        Gs{2} = S;
        Gs{2}.Converged = -1;
        d(2) = 0;
        details.dipres(2) = 0;
    ...
    j = length(d);
       ...
       if length(idlist) == size(xy,1) %give a list of classifications. Start with fit to their distributions
        ...
        S.PComponents = [length(id) length(nid)]./length(v);
            ...
            fprintf('GM Fit (Start with Classification) fail%s\n',lasterr);
            Gs{j} = S;
            Gs{j}.Converged = -1;
            Gs{j}.NlogL = NaN;
            d(j) = 0;
