...
DATA.fullvswitchmode.summary = 1;
DATA.iteratefit = 0;
DATA.interactive = 0;
...
if isstruct(V)
    newdata = 1;
    Vall = V;
    if ~isfield(Vall,'V') || isempty(Vall.V)
        return;
    elseif isinteger(Vall.V) && strcmp(convertarg,'convert');
...
%new data but figure is already up,     
    ...
    if isfield(Vall,'exptno')
        if isfield(DATA,'toplevel') && isfigure(DATA.toplevel) && isappdata(DATA.toplevel,'Expts')
            Expts = getappdata(toplevel,'Expts');
        else
            Expts = {};
        end
        if isfield(DATA.Expt,'exptno') && DATA.Expt.exptno == Vall.exptno && ~isempty(Expts)
            Expts = Expts;
...
if reclassifyall == 2
        DATA.Clusters = LoadCluster(DATA.name,DATA.exptno);
        for j = 1:length(ispk)
            if DATA.Clusters{ispk(j)}.quick || DATA.Clusters{ispk(j)}.trigdt == 4 ||  DATA.Clusters{ispk(j)}.manual == 2
                if DATA.Clusters{ispk(j)}.trigdt == 4
                    DATA.Clusters{ispk(j)}.trigdt = 0;
                end
...
if reclassifyall || refineall
    for j = 1:length(ispk)
        if refineall
            if size(Vall.V,1) ==1 && ispk ~= Vall.chspk
                p = Vall.chspk;
            else
                p = ispk(j);
            end
...
%quantification to save time.
        if ~isfield(DATA.Clusters{j},'marked')
            DATA.Clusters{j}.marked = 0;
        end
    end
...
elseif newdata == 2 %%Keep data.clusters if newdata == 2 - new probe
else
    DATA.Clusters = {};
end
...
if isempty(DATA.Clusters) && recluster ~= 1
    recluster = 0;
end

if recluster && length(DATA.Clusters) < DATA.probelist(DATA.probe(1)) && forcecluster ~= 2
...
if recluster
    ...
    if forcecluster == 0 %%used saved cluster
        DATA.cluster = DATA.Clusters{DATA.probelist(DATA.probe(1))};
...
if DATA.trigdt
    ...
    elseif DATA.trigdt == 4 %convolve with template
        ...
        if size(DATA.TriggerTemplate,1) > length(ispk)
            for j = 1:length(ispk)
            x(j,:) = conv(double(Vall.V(ispk(1),:)),DATA.TriggerTemplate(ispk(j),:));
            end
            x = mean(x,1);
            DATA.TriggerTemplate = DATA.TriggerTemplate(ispk,:);
        elseif length(ispk)  > 1
            for j = 1:length(ispk)
            x(j,:) = conv(Vall.V(ispk(j),:),DATA.TriggerTemplate(j,:));
            end
            x = mean(x,1);
        else
...
if gettrigtimes 
    ...
    if recluster == 2
        ...
        PlotXY(DATA.xy{1},DATA.clst);dbu
        th = DATA.cluster.Trigger;
    end
elseif recluster == 2
        [DATA, oldxyloaded] = LoadTrigTimes(DATA,[]);
        if oldxyloaded
...
DATA.cluster.eventrate = length(id)./length(smv);
...
%    DATA.chspk = chspk;
    ...
    if recluster %no need to calcuate GMfits etc
        [C, res.Evec, res.pcs] = CalcPCs(DATA,AllV,nprobepc);
        res.dipvals = 0;
    else
...
if verbose
    fprintf('Making Cut P%d %s (%s)\n',ProbeNumber(DATA),datestr(now,'HHMM:ss'),ClusterFile(DATA.name,DATA.Expt,str,'subdir',DATA.clustersubdir));
end
...
%if this clsuter previously used predefined eigenvectors, use those again        
        ...
        if (recluster == 1 || forceevec) && DATA.cluster.space(1) == 1
            ...
            [a,b, DATA.pcs] = CalcPCs(DATA, AllV, DATA.nprobepc);
            end
...
function DATA = GetGuiState(DATA, F)
    ...
    for f = {'handles' 'loadedClusterDetails' 'DataType' 'Expt' 'probeswitchmode' 'plotexptfit' 'logname' 'auto' 'checkclusters'}
        if isfield(X,f{1})
            DATA.(f{1}) = X.(f{1});
        end
    end
...
function [DATA, success] = LoadTrigTimes(DATA, checktimes, varargin)
    tic;
    forcecheck = 0;
    clst = {};
    p = DATA.probe(1);
...
function res = AutoCutAll(ispk, toplevel, Vall,DATA, args)
    ...
    if size(Vall.V,1) == 1 && strncmp(DATA.DataType,'Grid',4)
        ...
        for j = ispk
            ...
            a =  AllVPcs(name ,'nowatch',args{:},'tchan',j,'logfid',logfid,'saveautocut','toplevel',toplevel);
        end
...
function [DATA, id] = SaveClusters(DATA, outname,varargin)
    ...
    logname = [DATA.name '/' 'ClusterLogExpt' num2str(DATA.exptno) '.mat'];
...
figure(DATA.toplevel);
 ...
 function args = RetriggerDialog(a,b, fcn)
        ...
        a =uicontrol(F,'style','pushbutton','string','Go','callback',{@RetriggerDialog, 'go'});
        b = uicontrol(F,'style','pop','string','V|dVdt|accel|energy|Template','tag','trigtype');
        PlaceUi(a,b,'left');
        ...
        elseif strcmp(fcn,'go')
            F = get(a,'parent');
            trigtypes = {'threshold','dtthr','dtthr2','dtthr3','triggertemplate'};
            ...
            if trigtype == 5
                trigargs  = {'triggertemplate' DATA.Clusters{DATA.probe(1)}.MeanSpike.ms};
            else
...
function SaveCellList(DATA)
    cellfile = [DATA.name '/CellListA.mat'];
...
function Expt = LoadExpt(DATA, ei)
        ...
        if exist(exfile,'file') || exist(matfile,'file') %either will do
            ...
            if exist(exfile,'file')
                fprintf('Loading %s\n',exfile);
                [Trials, Expts] = APlaySpkFile(exfile,'noerrs','nospikes');
            else
                fprintf('Loading %s\n',matfile);
                [Trials, Expts] = APlaySpkFile(matfile,'noerrs','nospikes');
            end
            if isempty(Expts)
                Expt = [];
                return;
...
%        uimenu(sm,'Label','&Good','Callback',{@PCCluster, 'markgood'},'Tag','MarkGood');
 ...
 %       uimenu(hm,'Label','Var E Line','Callback',{@PCCluster, 7});
        ...
        a(3) = uimenu(sm,'Label','Reapply','CallBack',{@ProbeMenu, 'reapply'});
        a(4) = uimenu(sm,'Label','AutoCut','CallBack',{@ProbeMenu, 'autocut'});  
        a(5) = uimenu(sm,'Label','Simple cut (quickest)','CallBack',{@ProbeMenu, 'simple'});
        ...
        sm = uimenu(hm,'Label','When->New VullV','Tag','SwitchOptionMenu');
        ...
        uimenu(hm,'Label','&Summary Plot','Callback',{@OptionMenu, 'plotsummary'});
...
if strcmp(mode,'fit')
%    dip = FindDip(DATA.xy{1}(:,1),DATA.energy(1,:),'plot');
    DATA.plotexptfit = ~DATA.plotexptfit;
    set(a,'Checked',onoff{DATA.plotexptfit+1});
    if DATA.plotexptfit
...
function h = AddCellLabel(DATA,e,p)
    ...
    if iscell
        yo=0;
        h =text(xl(2),yl(1)+yo,sprintf('%d:Cell %d',p,cellid(1)),'color',DATA.colors{2},'horizontalalignment','right','verticalalignment','bottom');
    else
        [a,b] = find(squeeze(DATA.CellList(:,p,:)) > 0);
        if length(a)
            [c, id] = min(abs(a - e));
            cellid = DATA.CellList(a(id),p,b(id));
        h = text(xl(2),yl(1),sprintf('%d:(cell %d E%d)',p,cellid,a(id)),'color','r','horizontalalignment','right','verticalalignment','bottom');
        else
        h = text(xl(2),yl(1),sprintf(' %d',p),'color','k','horizontalalignment','right','verticalalignment','bottom');
        end
    end
...
function PlotProbeSpikes(DATA, Vall, p, spklist,npts,offset)
  ...
  for j = 1:length(DATA.Clusters{p}.next)
      if isfield(DATA.Clusters{p}.next{j},'times')
      tid = find(ismember(Vall.t(spklist),DATA.Clusters{p}.next{j}.times));
      else
          tid = [];
      end
      if length(tid)
...
%if only calculating metrics, don't call this       
        ...
        if DATA.watchplots && (needcl || force)
            PlotHistogram(DATA, []);
            ReplotPCs(DATA,[]);
        end
    end
%Unrefined Cluster N can steal events from refined cluster N-1, so need to appl one more time 
%and need to call Classify again anyway to apply refined boundayr
    if autorefine 
        DATA.autorefine = 0;
        DATA = ClassifyAll(DATA,1);
        DATA.autorefine = 1;
...
if ismember(cluster.shape,[1 2]) %cut with line
    ...
    [dp,dpfits, dpdetails] = Fit2Gauss(cluster, cluster.r(DATA.uid,:),DATA);
end
...
%    always neeeded.  other flags in quickmode detemine what fits are used.
    ...
    f = {'bestspace' 'bestd' 'auto' 'autotook' 'firstspace' 'firstbmi' 'bestll' 'gmdip' 'gmdipres' 'gmfit1d' 'gmfit' 'gmfits' 'aspectratio' 'next' 'pos' 'TemplateUsed' 'mumeanUsed' 'MeanSpike' 'eventrate' 'exptreadmethod'};
    for j = 1:length(f)
        if isfield(E,f{j})
            cluster.(f{j}) = E.(f{j});
        end
...
%buidling
    if quickmode.quick == 0 || ~isfield(C,'MeanSpike')
        cl.MeanSpike = PlotMeanSpike(DATA,'recalc');
        cluster.MeanSpike = cl.MeanSpike;
        if DATA.plotxyseq
            C = cluster;
...
function [out, TemplateUsed, DprimeUsed] = TemplatePlot(DATA, varargin)
    ...
    if isfield(C,'neednewtemplate') && C.neednewtemplate == 0
        set(DATA.toplevel,'Name',sprintf('Calculating Template Scores for C%d...',DATA.currentcluster));
    else
        set(DATA.toplevel,'Name',sprintf('Calculating New Templates for C%d...',DATA.currentcluster));
    end
    drawnow;
    ...
    if isfield(C,'neednewtemplate') && C.neednewtemplate == 0
        ...
        ms = C.TemplateUsed;
        mu = C.MeanSpike.mu;
        if size(DATA.chspk,1) == size(C.mumeanUsed,1)
        mu(DATA.chspk,:) = C.mumeanUsed;
        end
    elseif usemean == 1 || isempty(id)
        set(DATA.toplevel,'Name',sprintf('Calculating Templates from mean C%d...',DATA.currentcluster));
        ms = C.MeanSpike.ms;
        mu = C.MeanSpike.mu;
    else
        set(DATA.toplevel,'Name',sprintf('Calculating New Templates for C%d...',DATA.currentcluster));
   ...
   if isnan(sum(ms(:)))
    ...
    if usestd
        ...
        if size(AllV,1) > 2 %look at spatial derivatives
            ...
            TemplateScores(1,7,:) = DATA.StdTemplate(1,:) * squeeze(AllV(DATA.chspk,:,:) - meanV);
            TemplateScores(1,8,:) = DATA.StdTemplate(2,:) * squeeze(AllV(DATA.chspk,:,:) - meanV);
            TemplateScores(1,12,:) = squeeze(diff(DATA.StdTemplate(1,:),1,2)) * squeeze(diff(AllV(j,:,:),1,2));
        end
    end
...
for j = 1:size(plots,1)
    ...
    elseif ismember(DATA.plottype,[2 6 8 9])
        ...
        if j == 1
            text(0,1.2,sprintf('E%dP%d',DATA.exptno,p),'units','normalized');
        end
    end
    if ~ismember(j,[1 4])
        set(gca,'ytick',[]);
...
if isfield(DATA,'alldips')
   ...
   mysubplot(2,4,1);
   th = text(-0.4,1.1,sprintf('%s %.2f, dt%.2f, csd%.2f',num2str(p),b(1).*100,b(2).*100,b(3).*100),'units','norm');
elseif DATA.dvdt
       mysubplot(2,4,1);
       th = text(-0.4,1.1,sprintf('E%dP%d: dvdt',DATA.exptno,p(1)),'units','norm');
elseif DATA.csd
    th = text(-0.4,1.1,sprintf('E%dP%d: csd',DATA.exptno,p(1)),'units','norm');
else
   mysubplot(2,4,1);
   th = text(-0.4,1.1,sprintf('E%dP%d',DATA.exptno,p(1)),'units','normalized');
end
else
   mysubplot(2,4,1);
   if isfield(DATA.Expt.Header,'expname')
       name = DATA.Expt.Header.expname;
   else
       name = [];
   end
   th = text(0.5,0.0,sprintf('E%dP%d%s %s',DATA.exptno,p(1),DATA.probelabel,name),'units','normalized',...
...
h = NaN;    
    ...
    h = text(0,0,str,'units','Normalized','VerticalAlignment','Top','fontweight','bold');
...
options = optimset('MaxFunEvals',100000,'maxiter',maxiter,'display','off');
setappdata(DATA.toplevel,'fitparams',[]);
[dpa, afits] = MinimiseLine(guess, DATA,state);
nloop = 0;
