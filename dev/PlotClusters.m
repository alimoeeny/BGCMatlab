    ...
    elseif strncmpi(varargin{j},'Load',4)
        ...
        elseif isdir(DATA.name)
          if strncmpi(varargin{j},'LoadSpikes',8)
            ...
            DATA = LoadAll(DATA,[],'force', warnmode);
          end
            set(DATA.lstui,'string',DATA.strings);
        else
            fprintf('%s is not a directory\n',DATA.name);
...
elseif strncmpi(name,'CallAllVPcs',7)
    vname = FileName(DATA,DATA.exptid(DATA.currentpoint(1)),DATA.currentpoint(2),'FullV');
    if exist(vname,'file')
        AllVPcs(vname,'tchan',DATA.currentpoint(2),'nprobepc',1,'reapply');
    end
...
DATA.markcell.readmethod = 0;
DATA.markcell.quick = 0;
...
function d = DistanceMeasure(C, type, varargin)
        if type < 5
            d = C.mahal(type);
        elseif type == 5
...
%            PlotAllCellMean(DATA,'lineonly');
        ...
        elseif strcmpi(type,'allexpttune')
            ...
            for j = 1:length(cellid)
                ...
                hold off; 
                [a, AllExpt] = PlotSelectedExpts(DATA,'expts', exid(id), cid(id), 'cluster',clid(id)+1,'colors',colors(icolor),'cell',cellid(j));
        ...
        elseif strmatch(type,{'rateseqall' 'rateseqone'})
            ...
            for j = 1:length(cellids)
                ...
                p = 1+mod(b-1,np);
                for k = 1:length(a)
                    xcl = FindExcludedTrials(DATA,a(k),p(k),c(k),Clusters{a(k)}{p(k)});
                    if isempty(xcl)
                        xid = [];
                    else
                        xid = [Expts{a(k),p(k)}.Trials(xcl).id];
                    end
                    aExpts{k} = CountSpikes(Expts{a(k),1},Clusters{a(k)}{p(k)},c(k)+1,xid);
                    aExpts{k}.Header.cellnumber  = cellids(j);
                end
                AllExpts = cat(1,AllExpts,a);
                if length(a)
                a = PlotRateSequence(aExpts,'color',colors{j},scaling,'offset',j*2,'bytime','callback',{@HitTrial, cellids(j)});
            ...
            for j = 1:length(AllExpts)
                ...
                if mod(AllExpts(j),10) == 0
                    set(h,'linestyle','-');
                end
                if ~first(id)
                    h = text(t,yl(2),sprintf('%d %s',AllExpts(j),types{id}),'color',colors{id},'VerticalAlignment','bottom','HorizontalAlignment','right','rotation',90);
                    first(id) =1;
                end
            end
...
function [true, cellid] = isacell(DATA, row, p)
    ...
    function OptionMenu(a, b, fcn)
        ...
        elseif strcmp(fcn,'xcorrcells')
            ...
            toc
        elseif find(strcmp(fcn,{'xcorrallcells' 'xcorrallprobes' 'recalcxcorrcell' 'recalcxcorrall'}))
    ...
    function plots = PlotClusterPoints(C, uid, cid, varargin)
        ...
        for j = 1:length(cells)
            ...
            if length(id)
                ...
                if j > 1
                    set(plots(j),'ButtonDownfcn',{@ReplotXY, C.exptid, C.probe(1), j-1});
                end
            end
        end
...
function  h = AddTitle(DATA, C, titlemode)
    ...
    if titlemode == 2
        ...
        h=title(sprintf('E%.0fP%d %.2f',C.exptno,p,DistanceMeasure(C, DATA.mahaltype)));
    elseif titlemode == 1
    ...
    if isfield(C,'bestspace') && C.space(1) == 6
        if titlemode == 2
            c = DATA.NDSpaceChars(C.space(2));
            h=title(sprintf('E%.0fP%d %.2f ND%c',C.exptno,p,DistanceMeasure(C, DATA.mahaltype),c));
        elseif titlemode == 1
           h= title(sprintf('E%.0fP%d %.2f  %s %.2f',C.exptno,p,C.mahal(1),spstr,C.mahal(4)));
        else
           h= title(sprintf('P%d Ex %.0f Gm %.2f  %s (%.2f,%.2f for space %.0f)%.0f',p,C.exptno,C.mahal(1),spstr,C.mahal(4),C.bestspace(1),C.bestspace(2),C.sign));
        ...
        if titlemode == 2
            ...
            h=title(sprintf('E%.0fP%d %.2f%c',C.exptno,p,DistanceMeasure(C, DATA.mahaltype),c));
        elseif titlemode == 1
            h=title(sprintf('E%.0fP%d %.2f  %s %.2f',C.exptno,p,C.mahal(1),spstr,C.mahal(4)));
        else
...
function Expt = CountExptSpikes(Expt,C,clnum)
 ...
 function Expt = PlotExptCounts(DATA, e, p, cl, varargin)
     ...
     Expts = getappdata(DATA.toplevel,'Expts');
     clnum = cl+1;
     Expt = CountExptSpikes(Expts{e,1},Clusters{e}{p},clnum);
...
if DATA.plothist || DATA.refitgm
    ...
    [x, nsp] = PlotHist(C.xy,C,GMfits{ex}{p});
    hold on;
    scale = trapz(x,nsp);
    if isfield(C,'fitdpparams') && DATA.refitgm == 0
    ...
    elseif isfield(GMfits{ex}{p},'gmfit1d') 
...
%PlotSelectedExpts(DATA, ...)
    ...
    for j = 1:length(exid)
        args = {};
        Expt = CountExptSpikes(Expts{exid(j),1},Clusters{exid(j)}{pid(j)},clusterid(j));
        if strncmp(Expt.Header.expname,'image.orXob',11)
            args = {args{:} 'reverse'};
        end
        if Expt.Header.rc
...
%        DATA.CellList = CellList(1:nr,:);
        ...
        elseif exist('exptids','var')  % check expt list matched
            ...
            if length(ib) % new expts
                ...
                DATA.CellList(ib,:,:) = 0;
                DATA.CellDetails.exptids = exptids;
                CellDetails = rmfield(CellDetails,'trialids'); %rebuild below
            end
        end
...
function [f, isnew] = SetFigure(DATA, tag, varargin)
    ...
    if isnew
        if strfind(tag,'AllXY');
            ...
            uimenu(f,'Label','Keep','callback', {@ChangeTag, 'allxy'});
        ...
        elseif strfind(tag,'CellList');
            ...
            sm = uimenu(hm,'Label','&Cells','Tag','ProbePlotMenu');
            ...
            uimenu(hm,'Label','AllXY when change cell','Callback',{@PlotMenu, 'cells' 'watchallxy'});
    ...
    set(f,'UserData', DATA.toplevel);
        elseif strfind(tag,'AllSpikes')
            uimenu(f,'label','Keep','callback', {@ChangeTag, 'allspikes'});
            Expts = getappdata(DATA.toplevel,'Expts');
    ...
    if strcmp(type,'save')
       C = DATA.NewCut;
        cfile = [DATA.name '/Expt'  num2str(ex) 'ClusterTimes.mat'];
            afile = [DATA.name '/Expt'  num2str(ex) 'AutoClusterTimes.mat'];
        if exist(cfile,'file')
            load(cfile);
...
%thing is just to make changes separatley, not copy one to anaother
        if C.cluster > 1
            n = C.cluster-1;
            if ~isfield(C.next{n},'space') %new cluster - cut in same space ast 1
            ...
            elseif ~isfield(ClusterDetails{p},'next') || ~isfield(ClusterDetails{p}.next{n},'xy')
           ...
           Clusters{p}.savetime(1) = now;
            EClusters{e}{p} = CopyFields(EClusters{e}{p},C,...
                'shape','crit','xyr','sign','angle','savetime','times');
            Clusters{p}.manual = 2;
    ...
    elseif strncmp(type,'ellipse',7) || strcmp(type,'lines')
        ...
        PlotClusterXY(DATA,C,'xydata',XY);
...
%    Clusters{e}{p}.xy = C.xy; Shouldn't have to change xy any more
    ...
    s = sprintf('%d/%d Spikes',nspk,length(C.clst));
    title(s);
    fprintf('%s\n',s);
...
function DATA = PlotExptsProbe(DATA, type);
    ...
    for j = 1:length(expts)
        ...
        elseif strmatch(type,{'AllprobeXY' 'AllTemplateXY'})
            ...
            xyplots = PlotClusterXY(DATA,Clusters{eid}{p},'shorttitle','tight');
            title(sprintf('E%dP%d:%.1f',DATA.exptid(eid),p,DistanceMeasure(Clusters{eid}{p},DATA.mahaltype)));
...
function [C, DATA] = CheckClusterLoaded(DATA,e)
    ...
    newe = 0;
for j = 1:length(e)    
    if e(j) > length(C) || isempty(C{e(j)})
        C{e(j)} = LoadCluster(DATA.name,e(j),'rawxy','alltimes');
        for k = 1:length(C{e(j)})
        C{e(j)}{k}.exptid = e(j);
        if ~isfield(C{e(j)}{k},'MeanSpike')
            C{e(j)}{k}.MeanSpike = [];
        end
        end
        newc = 1;
    end
    if e(j) > size(E,1) || isempty(E{e(j),1})
        [E{e(j),1}, DATA] = LoadExpt(DATA,e(j));
        newe = 1;
    end
end
...
if newc
    setappdata(DATA.toplevel,'Clusters',C)
end
if newc || newe
    set(DATA.toplevel','UserData',DATA);
    DATA.modified = 1;
...
function DATA = PlotAllProbe(DATA, type)
    ...
    axdata.toplevel = DATA.toplevel;
    if allprobes && isfield(DATA,'ArrayMap')
        for j = 1:np
            plotpos(j) = DATA.ArrayMap.X(j) + (DATA.ArrayMap.Y(j)-1).*nr;
        end
    else
        plotpos = 1:np;
    end
     ...
     for j = 1:np
        ...
        if strmatch(type,{'allspks' 'allcellspks' 'selectspks' 'exptspks' 'allprobespks'})
            ...
            [h, x] = QuickSpikes(DATA, AllSpikes{eid,pid(j)},C,'showmean');
            axdata.lines = h;
...
function ReplotXY(a,b,eid, probe,cid)
    ...
    if DATA.plottrighist
        AddTrigHist(Clusters{eid}{probe});
    end
    set(gca,'Xtick',[],'Ytick',[],'ButtonDownFcn',callback);
...
function DATA = PlotAllProbeXY(DATA,varargin)
    ...
    for j = 1:np
        ...
        set(gca,'Xtick',[],'Ytick',[]);
        if DATA.plottrighist
            AddTrigHist(Clusters{eid(j)}{cid(j)});
        end
...
function missed = MissedCell(DATA, pos)    
    ...
    if isacell(DATA,e,p) == 0 && ((d > 2 && abs(fitd) > 1) || abs(fitd) > 3) && DATA.peakdiff(e,p) == 0 && DATA.dropi(e,p) > 1
        missed = 1;
    else
        missed = 0;
    end
...
function PlotAllCellXY(DATA)
    ...
    for j = 1:length(eid)
        mysubplot(nr,nc,j);
        hold off; 
        plots = PlotClusterXY(DATA,Clusters{eid(j)}{cid(j)},'shorttitle','cellid',clid(j));
        if clid(j) ~= 1 
            np = clid(j)+1;
            if length(plots) >= np
...
function DATA = InitInterface(DATA)
    ...
    bp(3) = 0.3;
    bp(2) = bp(2)+ 1./nr;
    uicontrol(gcf,'style','text','string','Distance Measure',...
        'units', 'norm', 'position',bp,'value',1);
    bp(1) = bp(1)+bp(3)+hspace;
    bp(3) = 0.15;
    ...
    sm = uimenu(hm,'Label','CrossCorr','Tag','xCorrPlotMenu');
    uimenu(sm,'Label','Cells all Expts','Callback',{@OptionMenu, 'xcorrallcells'});
    uimenu(sm,'Label','Cells in current Expt','Callback',{@OptionMenu, 'xcorrcells'});
    uimenu(sm,'Label','Selected Probes','Callback',{@OptionMenu, 9});
    ...
    [DATA.fig.clusters, isnew] = GetFigure(DATA.tag.clusters);
    if isnew
...
function AddPlotMenu(sm, type)
    ...
    uimenu(sm,'Label','&Expt All Spks +xy','Callback',{@PlotMenu, 'probes', 'exptall'});
    ...
    uimenu(sm,'Label','All X&Y This Probe','Callback',{@PlotMenu, 'probes', 'AllprobeXY'});
...
function AddExptList(hm, DATA)
    ...
    uimenu(hm,'Label','None','Callback',{@PlotMenu, 'markexpts' 'none'});
    if isfield(DATA,'expnames')
        ...
        for j = 1:length(names)
            ...
            if length(id) == 1
                uimenu(hm,'Label',KnownExpts{id,1},'Callback',{@PlotMenu, 'markexpts', names{j}});
            else
                uimenu(hm,'Label',names{j},'Callback',{@PlotMenu, 'markexpts', names{j}});
            end
        end
    end
    uimenu(hm,'Label','relist','Callback',{@PlotMenu, 'markexpts' 'relist'});
...
function DATA = LoadAll(a,b, type, varargin)
    ...
    j = 1;
    while j <= length(varargin)
        if strncmpi(varargin{j},'nowarn',5)
            msgmode = 2;
        end
        j = j+1;
    end
        ...
        for j  = 1:length(d)
            ...
            elseif strfind(d(j).name,'ClusterTimes.mat') & isempty(strfind(d(j).name,'OldClusterTimes.mat'))
                ...
                if isempty(id) || d(j).datenum > DATA.lastcheck%new file
                    ...
                    [C, details] = LoadCluster(DATA.name, eid,'getxy','alltimes');
                end
            end
        end
...
%if only cluster 2 has changed, ctime for cluster 1 does not. But savetime does.                        
                ...
                smrname = regexprep(DATA.name,[monk '/M([0-9]*)(.*)'],['$0/' monk 'M$1']);
   ...
   if ~strcmp(type,'ifnew')   
    ...
    DATA = LoadCellFile(DATA);
    DATA = LoadExtra(DATA,0);
    mapfile = [DATA.name '/ArrayMap.mat'];
    if exist(mapfile)
        load(mapfile);
        DATA.ArrayMap = Array;
    end
...
function h = DrawBox(ex, p, imtype, varargin)
    ...
    if length(ex) > 1 || length(p) > 1
        square = [min(p)-.5 max(p)+0.5 max(p)+0.5 min(p)-0.5 min(p)-0.5; min(ex)-0.5 min(ex)-0.5 max(ex)+0.5 max(ex)+0.5 min(ex)-0.5];
    else
    square = [-.5 0.5 0.5 -0.5 -0.5; -0.5 -0.5 0.5 0.5 -0.5];
...
if length(Expts)
    for j = 1:size(Expts,1)
        for k = 1:size(Expts,2)
            ctimes(j,k) = Expts{j,1}.Header.CreationDate;
        end
    end
    estart = min(ctimes(ctimes > 0));
    for j = 1:size(Expts,1)
        for k = 1:size(Expts,2)
            Expts{j,k}.Header.timeoffset = (ctimes(j,k)-estart).*24.*60.*60;
        end
    end
    setappdata(DATA.toplevel,'Expts',Expts);
end
        ...
        for j = 2:length(h)
            if h(j) > 0 & ishandle(h)
                ...
                if j > 2
                    ...
                    details.meanh(j) = plot(C.next{j-2}.MeanSpike.ms(pos(2),:),'-','color',color,'linewidth',2);
                    end
                else
                    details.meanh(j) = plot(C.MeanSpike.ms(pos(2),:),'-','color',color,'linewidth',2);
                end
            end
        end
        if length(h) < 2 
            details.meanh(2) = plot(C.MeanSpike.ms(pos(2),:),'-','color',DATA.colors{2},'linewidth',2);
        end
    end
    plot([1 1],[0 -1],'k-','linewidth',2); %scale bar
    axdata = get(gca,'UserData');
    axdata.toplevel = DATA.toplevel;
    axdata.probe = p;
    axdata.eid = e;
...
function stopped = SpoolSpikes(DATA, pos, varargin)
    ...
    for j = 1:length(Trials)
        ...
        ispk = find(C.times > Trials(j).Start(1)./10000 & C.times < Trials(j).End(end)./10000);
        PlotSpikes(DATA, pos, ispk, Spks, C, 'fixy',yl,args{:});
        drawnow;
        end
        stopped = get(stopbtn,'value');
        if stopped
