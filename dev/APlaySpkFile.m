                        ...
                        for c = 1:length(SpkTimes(aid(2)).Clusters);
                            if c > length(bClusters) || isempty(bClusters{c})
                                ...
                                if isfield(bClusters{c},'excludetrialids')
                                    Clusters{c}.excludetrialids  = cat(2,Clusters{c}.excludetrialids, bClusters{c}.excludetrialids);
                                end
            ...
            for j = 1:length(Clusters)
                ...
                if isfield(Clusters{j},'times')
                    ...
                    if isfield(Clusters{j},'mahal')
                        Spks{j}.dips = [Clusters{j}.mahal(4) Clusters{j}.mahal(1)];
                    ...
                    if isfield(Clusters{j},'excludetrialids')
                        Spks{j}.excludetrialids = Clusters{j}.excludetrialids;
                    else
                        Spks{j}.excludetrialids = [];
                    end
                    if length(Spks{j}.excludetrialids) > 0
                        a = max(Spks{j}.excludetrialids);
                    end
                end
            end
            end
...
%fixed/crashed files
    ...
    fsid = find(Events.codes(id,1) ==  5 & Events.times(id) < estimes(eid(end)));
...
if length(fsid) == length(bsid)
  ...
  for j = 1:length(fsid)
      ...
      if length(id) == 0
          Trials.Result(j) = -1;
     elseif j <length(fsid) && length(id) 
...
function ix = FindExptn(Expts, trial, ix)
if isfield(Expts,'firsttrial') & isfield(Expts,'lasttrial')
    lastix = ix;
    ix = find([Expts.firsttrial] < trial+2 & [Expts.lasttrial] > trial);
    if ix
...
% be a conversion to stimulus type
        ...
        if isfield(AllTrials,'Stimseq') && length(AllTrials.Stimseq{igood(k)}) > 1 && isfield(Expt,'e1vals') && ~isempty(Expt.e1vals)
...
%regular tuning curves
    if strmatch(Expt.Stimvals.et,{'xo' 'yo' 'Op' 'Pp'},'exact')
