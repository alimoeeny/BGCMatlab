...
for j = 1:length(d)
        ...
        if length(sid)
            ...
            if isempty(expts) || ismember(ex,expts)
                ...
                name = [path '/' d(j).name];
                if classifyfromcluster ==2 
                    if exist(cfile)
                        load(cfile);
                        if (isfield(Clusters{p},'quick') && Clusters{p}.quick) || ... 
                                (isfield(Clusters{p},'manual') && Clusters{p}.manual == 2)
                            res.cls{ns} = ClassifyFromCluster(name,Clusters,'quantify',args{:});
                        end
                    end
                elseif classifyfromcluster == 1
                    res.cls{ns} = ClassifyFromCluster(name,Clusters,args{:});
                else
                res.cls{ns} = AllVPcs(name, args{:});
                end
...
function res = ClassifyFromCluster(name, Clusters, varargin)
    ...
    res = AllVPcs(name,'tchan',p,'reapply',Clusters{p},args{:},argon{:});
...
if isfield(FullV,'initialloadtime')
    fprintf('Int Conversion took took %.2f\n',FullV.loadtime-FullV.initialloadtime);
end
ts = now;
res = AllVPcs(FullV,args{:});
if ~iscell(res)
...
clear FullV; %%SHouldn't matter. But there is a leak somwhere
...
function Ex = LoadExpt(name, ei, rebuild, logfid)
 ...
 function Summary = CheckExptList(X)
        ...
        for j = 1:length(X.cls)
            Summary.gmdprime(j) = X.cls{j}.cluster.gmdprime;
            Summary.mahal(j,:) = X.cls{j}.cluster.mahal;
        end
