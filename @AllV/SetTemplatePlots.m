...
%AllV.SetTemplatePlots sets the template spaces to plot, depending on probes, cluster
%version
    ...
    if DATA.cluster.version > 1.11
        if isfield(DATA,'TemplateScores') && size(DATA.TemplateScores,2) > 18
            DATA.tmplots(3,:) = [1 19]; %p2(trigger) vs p3 if p4 present, else sum
        else
            DATA.tmplots(3,:) = [1 10]; % else sum
        end
    else
...
% tmplots 17:24 is for Std Template Plots    
    ...
    if np > 2
        ...
        DATA.tmplots(20,:) = [nt+1+nt/2 nt+nt/2]; %p2T2dt p1T1dt
        DATA.tmplots(18,:) = [nt+1 2*nt-1]; %p2(trigger)T1 vs
    end
...
%41:48  is Template plot 3
    DATA.tmplots(41,:) = [2 5];
    DATA.tmplots(42,:) = [1 5];
    DATA.tmplots(43,:) = [2 4];
    DATA.tmplots(44,:) = [3 19];
    DATA.tmplots(45,:) = [5 12];
    DATA.tmplots(46,:) = [4 19];
    DATA.tmplots(47,:) = [17 18];
    DATA.tmplots(48,:) = [3 4];
