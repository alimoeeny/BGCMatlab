...
%each expt
%
%RunallVpcs(...,'spikes') load spks files rather than .fullv files  
%
%
%RunallVpcs(...,'trycatch') wraps each call to AllVPcs inside a try/catch
%      statement, so that a failure on one expt does not stop others
%      completting.  CheckExcecptions run on the return argument will list 
%      errors
%
% Clusters{tlist(j)} is applied to probe tlist(j)AllV.
%
...
%RunallVpcs(cmds,'runinteractive')  runs the listed commandsre.
%see also CheckExceptions,  AllV.CheckLogs
...
while j <= length(varargin)
    ...
    elseif strncmpi(varargin{j},'skip',4)
        j = j+1;
        nskip = varargin{j};
    elseif strncmpi(varargin{j},'spikes',6)
        usespikes = 1;
        args = {args{:} varargin{j}};
    else
        args = {args{:} varargin{j}};
...
if usespikes
    ...
    res.prefix = name;
    for j = 1:length(d)
        eid(j) = GetExptNumber(d(j).name);
    end
    eid= unique(eid(eid>0));
    if ~isempty(expts) %user specified expt list
        expts = intersect(expts,eid);
    else
        expts = eid;
    end
    res.prefix = name;
    for j = 1:length(expts)
        ...
        res.name{ns} = name;
        filenames{ns} = name;
        res.exptlist(ns) = expts(j);
        if ~isempty(logfile)
            PrintMsg(logfile,'Requested %s Spikes %s',filenames{ns},datestr(now));
        end
        ns = ns+1;
    end
    ...
    for j = 1:length(d)
        ...
        if length(sid)
            ...
            if isempty(expts) || ismember(ex,expts)
                if ~isempty(logfile)
                    PrintMsg(logfile,'Requested %s (FullV) on %s for %s %s',d(j).name,gethostname,GetUserName,datestr(now));
                end
                name = [path '/' d(j).name];
                res.cls{ns} = [];
                %              fclose all;
...
if parallel && ns > 1
    ...
    parfor  (j = 1:ns-1)
        t = mygetCurrentTask();
        fprintf('Expt%d worker id is %d (at %s)\n',res.exptlist(j),t.ID,datestr(now));
        PrintMsg(logfile,'Worker %d Processing %s at %s',t.ID,filenames{j},datestr(now));
        cls{j} =  ProcessSuffix(filenames{j}, res.exptlist(j), args{:});
        PrintMsg(logfile,'Completed %s at  %s',filenames{j},datestr(now));
        workers(j) = t.ID;
    end
    res.cls = cls;
    res.workerid = workers;
elseif ns > 1
    args = {args{:} 'nowatch' 'nocheck' 'verbose'};
    workers = [];
    for  (j = sortid(:)')
        t = mygetCurrentTask();
        fprintf('Expt%d worker id is %d (at %s)',res.exptlist(j),t.ID,datestr(now));
        PrintMsg(logfile,'Processing %s at %s\n',filenames{j},datestr(now));
        cls{j} =  ProcessSuffix(filenames{j}, res.exptlist(j), args{:});
        PrintMsg(logfile,'Completed %s at  %s',filenames{j},datestr(now));
        workers(j) = t.ID;
    end
 ...
 res.end = now;
 res.args = args;
 t = mytoc(res.starts(1));
 if t > 3600
     tstr = sprintf('%.1f hrs',t./3600);
 elseif t > 60
     tstr = sprintf('%.1f min',t./60);
 else
     tstr = sprintf('%.1f sec',t);
 end
     
 PrintMsg(logfile,'Expts%s Took %s on %s at %s. Started at %s',sprintf(' %d',expts),tstr,gethostname,datestr(now),datestr(res.starts(1)));
 if logfid > 0
     fclose(logfid);
 end
...
while j <= length(varargin)
    ...
    elseif strncmpi(varargin{j},'spikes',6)
        usespikes = 1;
    elseif strncmpi(varargin{j},'savespikes',7)
        savespikes = 1;
        args = {args{:} varargin{j}};
    elseif strncmpi(varargin{j},'saveclusters',7)
        savespikes = 2;
        args = {args{:} varargin{j}};
    elseif strncmpi(varargin{j},'template',6)
        ...
        template = varargin{j};
        cargn = j;
    elseif strncmpi(varargin{j},'trycatch',6)
        trycatch =1;
    else
        args = {args{:} varargin{j}};
    end
...
outname = path;
if usespikes
    outname = sprintf('%s/Expt%dSpikes',path,ex);
elseif exist(outname,'file')
    ts = now;
    fprintf('%s Exists\n',outname);
    d = dir(outname);
else
...
if savespikes == 0 
    cprintf('red','!!!!\n!!!!  ''savespikes/clusters'' Not in arg list. Be sure you don''t want to save the clusters\n!!!!\n');
end
...
if checkspikes
    ...
    for j = 1:length(id)
        res.cls{j} = AllVPcs(outname,'tchan',id(j),args{:}, runargs{:});
    end
    end
    return;
end
...
if ~isempty(probes)
    ...
    for j = 1:length(probes)
        if trycatch
            try
                res.cls{j} = AllVPcs(FullV,'tchan',probes(j), args{:},runargs{:});
                catch ME
                    res.cls{j}.errstate = ME;
                    CheckExceptions(ME);
            end
        else
                res.cls{j} = AllVPcs(FullV,'tchan',probes(j), args{:},runargs{:});
...
ts = now;
if printcmd
    res = sprintf('AllVPcs(''%s''',outname);
    for j = 1:length(args)
        if ischar(args{j})
            if strcmp(args{j},'Expts')
                j = j+1;
            else
                res = [res sprintf(', ''%s''',args{j})];
            end
        end
    end
    for j = 1:length(runargs)
        if ischar(runargs{j})
            res = [res sprintf(', ''%s''',runargs{j})];
        end
    end
    res = [res ');'];
    return;
...
if trycatch
    try
        res = AllVPcs(outname,args{:},runargs{:});
    catch ME
        res.errstate = ME;
        CheckExceptions(res);
    end
    ...
    if ~verLessThan('matlab','8.3') && strcmp(computer,'PCWIN64')
       [x,y] = memory;
       fprintf('Memory Usage: %.3fGb/%.3gGb',x.MemUsedMATLAB./(1024.*1024.*1024),y.PhysicalMemory.Total/(1024.*1024.*1024))
    end
try
    for j = 1:length(nid)
...
function Ex = LoadExpt(name, ei, rebuild, logfid)
        ...
        if exist(matfile) && (~exist(exfile,'file') || rebuild)
            PrintMsg(logfile,'Building %s',exfile);
...
function res = RunSelectedCommands(X, skip, varargin)
    ...
    allvargs = {allvargs{:} 'nodefaults' 'runallv'};
...
% 128 = needed becuase of dates/files
            if ismember(X.actimage(e,p),[1 3 7 8 10 128]) & ismember(cellim(e,p),docells)
                ...
                cmd = ['AllVPcs(' vname, ',''tchan'',' num2str(p) ', ''nodefaults'', ' sprintf('%s, ', allvargs{:}) ') %%' num2str(res.reason(nc)) cellchr reasons{res.reason(nc)} ];
                fprintf([cmd '\n']);
                res.cmds{nc} = cmd;
...
%                    cls{j}{k} = X.args{id(k)};
        ...
        for j = 1+skip:length(X.args)
            res{j} = AllVPcs(X.args{j}{:},allvargs{:},'nodefaults');
            res{j}.saved = 0;
            if X.interactive
    ...
    function res = RunCommandList(X, varargin)
        ...
        X = X(id);
        allnames = allnames(id);
        addargs = {addargs{:} 'nodefaults' 'runallv'};
    ...
    for k = 1:length(names)
        ...
        for j = id'
            ...
            for k = 1:length(X(j).probes)
                cmd = sprintf(['AllVPcs(FullV, ''tchan'',' num2str(X(j).probes) ', ''nodefaults'', ' sprintf('%s, ', X(j).args{:}) ')']);
                fprintf([cmd '\n']);
                if checkmode == 0
                    if length(logfile)
                        logfid = fopen('logfile','a');
                    ...
                    res{n} = AllVPcs(FullV, 'tchan', X(j).probes(k), X(j).args{:}, 'nodefaults', addargs{:});
                    fclose all;
                    res{n}.name = name;
                    res{n}.args = X(j).args;
