...
while j <= length(varargin)
    if strncmpi(varargin{j},'all',3)
        loadall = 1;
    elseif strncmpi(varargin{j},'clusters',7)
...
if isdir(backupdir)
    ...
    for e = 1:length(expts)
        ...
        if length(expts) > 1 && loadall == 0 %load  last backup backups
            d = d(b(1)); %most recent
            b = 1;
        elseif loadall
            fprintf('Loading Expt %d\n',expts(e));
        end
        for j = 1:length(d)
            x = load(d(b(j)).name);
            if isfield(x,'Clusters')
                for k = 1:length(x.Clusters)
                    x.Clusters{k} = rmfields(x.Clusters{k},{'gmfit2d' 'gmfit2dman' 'gmfit1d'});
                end
...
if strcmp(plottype,'savetime')
    ...
    if length(expts) == 1
        ...
        h(1) = imagesc(squeeze(xcorrs(:,:,end)));
    end
...
for e = 1:length(D.Backups)
    for s = 1:length(D.Backups(e).Clusters)
        if iscell(D.Backups(e).Clusters{s})
            C = CellToStruct(D.Backups(e).Clusters{s});
            D.Backups(e).Clusters{s} = C;
        end
    end
end
for e = 1:length(D.Backups)
    for s = 1:length(D.Backups(e).Clusters)
        B(e).Clusters(s).Clusters = D.Backups(e).Clusters{s};
    end
end
D.Backups = B;
SetData(D);

function Summarize(a,varargin)

probes = 1;
j = 1;
while j <= length(varargin)
    if strncmpi(varargin{j},'probes',4)
        j = j+1;
        probes = varargin{j};
    end
    j = j+1;
...
FixData(D);
return
if isfigure(a)  && isappdata(a,'Summary')
    Summary = getappdata(a,'Summary');
else
for e = 1:length(D.Backups)
    savetime = [];
    ncut = [];
    for s = 1:length(D.Backups(e).Clusters) %sessions
        for p = 1:length(D.Backups(e).Clusters{s})
            C = D.Backups(e).Clusters{s}{p};
            savetime(s,p) = C.savetime(end);
            ncut(s,p) = C.ncut;
        end
    end
    [a,b] = find(diff(ncut)~=0);
    Summary.savetimes{e} = savetime;
    Summary.ncuts{e} = ncut;
end
setappdata(D.toplevel,'Summary',Summary);
end
GetFigure('Backup Summary');
hold off; 
for e = 1:length(Summary.savetimes)
    [a,b] = find(diff(Summary.savetimes{e})~=0);
    for j = 1:length(probes)
        id = find(b == probes(j));
        aid = [a(id) a(id)+1];
        aid = a(id);
        x = Summary.savetimes{e}(aid,j);
        y = Summary.ncuts{e}(aid,j);
        [x,id] = sort(x);
            y = y(id);
        plot(x,y,'o-');
        hold on;
        for k = 2:length(id)
            if y(k-1) > 0 && abs(y(k)-y(k-1))./sum(y(k-1:k)) > 2              
                strs{1} = '';
                strs{2} = '';
                fprintf('E%dP%d ncut %d (%s) ->%d (%s)on %s\n',e,probes(j),y(k-1),strs{1},y(k),strs{2},datestr(x(k)));
            end
        end
    end
    datetick('x');
...
if D.oneexpt > 0
    ...
    if length(eC) >= p
        ...
        j = eid-1;
        while j > 0 
            eC = D.Backups(e).Clusters{j};
            if length(eC) >= p
                fprintf('E%dP%d Backup%d No cluster for Probe %d. Using Backup %d\n',e,p,eid,p,j);
                Cb = D.Backups(e).Clusters{j}{p};
                j = 0;
            end
            j = j-1;
        end
...
if D.oneexpt > 0
    PlotHistory(Ca, D.Backups(e));
end


function PlotHistory(C, B)
p = C.probe(1);
for j = 1:length(B.Clusters)
    if length(B.Clusters{j}) >= p;
        CC = B.Clusters{j}{p};
        nspks(j) = CC.ncut;
        ts(j) = CC.savetime(end);
        nclusters(j) = 1+length(CellToMat(CC.next));
        isolation(j) = CC.isolation(2);
    end
end
GetFigure('ClusterHistory');
hold off;
plot(nspks./C.ncut);
hold on;
plot(nclusters);
