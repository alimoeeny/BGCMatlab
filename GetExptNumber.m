...
elseif isfield(name,'Header')
    ...
    if isfield(E.Header,'bysuffix')
        bysuffix = E.Header.bysuffix;
    else
        bysuffix = 0;
    end
        
    if isfield(E.Header,'suffix') && bysuffix
        n = E.Header.suffix;
    elseif isfield(E.Header,'exptno') && bysuffix == 0 
        n = E.Header.exptno;
...
if isstruct(name)
    if isfield(name,'exptno')
        n = name.exptno;
        return;
    elseif isfield(name,'expt') && isnumeric([name.expt])
        n = [name.expt];
        return;
    elseif isfield(name,'suffix')
        n = name.suffix;
        return;
    elseif isfield(name,'eid') && isnumeric(name.eid)
        n = name.eid;
        return;
    elseif isfield(name,'name')
        if length(name) > 1
            [n, alln] = GetExptNumber({name.name},varargin{:});
            return;
        end
        name = name.name;
    end
...
elseif regexp(name,'^Cluster [0-9]+ E[0-9]+')
    ...
    if isempty(id)
        id = regexp(name, '\.[0-9]+idx.mat');
    end
    if isempty(id)
        id = regexp(name, '\.[0-9]*.smr');
    end
    if isempty(id)
        id = regexp(name, '\.[0-9]*.spkblk');
    end
    if ~isempty(id)
