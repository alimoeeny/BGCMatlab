...
while j <= length(varargin)
    if strncmpi(varargin{j},'addmains',6)
        addmains = 1;
    elseif strncmpi(varargin{j},'alltrials',6)
        loadalltrials = 1;
...
if ~exist('Expt','var') && exist('AllExpt','var')
    ...
    if loadlfp
        AllExpt.Expt = LoadLFP(AllExpt.Expt,varargin{:});
    end
    if loadem
...
if exist('Expts','var') && iscell(Expts)
    for j = 1:length(Expts)
        Expts{j}.Header.loadname = name;
    end
    Expt = Expts;
    return;
end
...
if ~isfield(Expt.Header,'rc')
    Expt.Header.rc = 0; %if(rc) fails, but records uncertainty
end
...
if isfield(Expt.Stimvals,'ed') && isempty(Expt.Stimvals.ed)
    Expt.Stimvals.ed = 0;
end
if loadem
