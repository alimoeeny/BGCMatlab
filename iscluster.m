...
if nargin == 1
    if isempty(C)
        res = 0;
    elseif iscell(C) && iscell(C{end}) && isfield(C{end}{end},'mahal') %Cell array of cluster arrays
        res = 3;
    elseif iscell(C) && isfield(C{end},'mahal')
        res = 2;
    elseif isfield(C,'toplevel') %A PlotClusters DATA struct
        res = 0;
    elseif isfield(C,'mahal')
        res = 1;
    elseif isfield(C,'xy') && isfield(C,'Evec') %CellDetails
        res = 4;
    end
elseif isempty(c)
    res = 0;
...
elseif iscell(C) && isstruct(C{1})
    res = 0;
    if length(C) >= c && isfield(C{c},'shape')
        res = 1;
        return;
    end    
else
    if ~iscell(C) | isempty(C) | size(C,1) < c | size(C,2) < p | isempty(C{c,p})
        res = 0;
    elseif ~isfield(C{c,p},'params') | ~isfield(C{c,p},'x') %not a combine Cluster
        res = 0;
        if isfield(C{c,p},'space') && ~isempty(C{c,p}.space)
            res = 3;
        end
