...
pfile = BuildFileName(dirname,'paramfile');

if exist(pfile,'file')
    pd = dir(pfile);
    txt = scanlines(pfile);
    expt = binoc.InterpretLine(txt);
end
rffile = dir2name(dirname, 'rf');
d = dir(rffile);
if exist(rffile,'file') && ~rebuild    
    if ~isempty(expt) && pd.datenum > d.datenum
        fprintf('Rebuilding %s because %s is newer\n',rffile,pfile);
        rebuild =1 ;
    else
    try
    load(rffile);
...
for j = 1:length(d)
    ...
    if isfield(Expt.Trials,'VisualArea')
        idxarea = setdiff(unique(Expt.Trials.VisualArea),{'NotSet' 'Unknown'});
        if ~isempty(idxarea) && isempty(areas{j})
            areas{j} = idxarea{1};
        end
    end
...
if isempty(areas)
    ...
    va = setdiff(unique(areas),{''}); %areas named in pen file
end
if size(rfs,2) < 6
    cprintf('red','No Penetration information for %s\n',dirname);
    return;
end
gid = find(rfs(:,6) > 0);
pe = median(rfs(gid,6));
...
if isfield(pen,'files') && isfield(pen,'visualarea')
    ...
    if nf
        vb = setdiff(unique(areas),{'' 'Unknown'});
        if isempty(va)
            va = vb;
        elseif ~isempty(vb) && ~sum(strcmp(va,vb))
...
va = setdiff(va,{''});
if isfield(expt,'VisualArea') %force
    therf.area = expt.VisualArea;
elseif isempty(va)
    therf.area = 'unknown';
else
    therf.area = va{1};
...
if isfield(pen,'pos') && sum(abs(pen.pos)) > 0 && ~forcepen
