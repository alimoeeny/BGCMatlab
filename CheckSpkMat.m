...
% CheckSpkMat(name, 'bsextra') Finds "orphan" stimOn/Off pulses. E.g.
% lemM279.29
checkmode = 1;
result = 1;
details.errors = {};
...
while j <= length(varargin)
    ...
    elseif strncmpi(varargin{j},'mains',5)
        checkmode = 2;
    elseif strncmpi(varargin{j},'store',3)
        checkmode = 'store';
...
elseif strcmp(checkmode,'store')
    txt = cellstr(CSer.text);
    sid = find(strncmp('sb+',txt,3));
    oid = find(strncmp('sb-',txt,3));
    id = find(strncmp('sb',txt,2));
    for j = 1:length(id)-1
        if strncmp(txt{id(j)},'sb-',3)
            onoff = 0;
        elseif strncmp(txt{id(j)},'sb+',3)
            onoff = 1;
        else
            onoff = NaN;
        end
        storing(id(j):id(j+1)-1) = onoff;            
    end
    storing(length(CSer.times)) = NaN;
    hold off;
    plot(CSer.times, storing);
    hold on;
    id = find(strncmp('bss',txt,3));
    plot(CSer.times(id),1,'r.')
    id = find(strncmp('bsn',txt,3));
    plot(CSer.times(id),0,'b.')
elseif checkmode == 1
