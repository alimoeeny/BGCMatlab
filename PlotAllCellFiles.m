...
%DATA.toplevel may be a parent figure
%DATA.allfig is this figure
DATA.tag.allexpts = 'AllCellPlot';
DATA.toplevel = 0;
DATA.plottype = 'means';
DATA.plot.onetype = 'default';
plotargs = {};
plottype = 'rates';
parentfigure = 0;
...
for j = 1:length(A)
    if isfield(A{j},'Header') && isfield(A{j},'plotres') && isfield(A{j}.plotres,'Header')
...
if ~isfield(DATA,'show') || ~isfield(DATA.show,'isolation')
    DATA.show.isolation = 0;
end
...
if sum(strcmp('parallel',strargs))
    fprintf('Building Initial Plot Results (Parallel)\n');
parfor j = 1:length(DATA.Expts.Spikes)
    ...
    AllExpts{j}.plotres(1).cellid = Expt.Header.cellnumber;
        ...
        fprintf('Building Initial Plot Results\n');

for j = 1:length(DATA.Expts.Spikes)
    Expt = All2Expt(DATA.Expts,j,'all');
    if plotfirst
        AllExpts{j}.plotres = PlotExpt(Expt,'rcnmin',5,'fbox',varargin{:});
        drawnow;
    end
    AllExpts{j}.Header = Expt.Header;
    AllExpts{j}.Trials = Expt.Trials;
...
DATA.nexpts = length(AllExpts);
setappdata(DATA.allfig,'Expts',DATA.AllExpts);
if plotfirst
    PlotAllCells(DATA,[], 'rates');
else
    PlotAllCells(DATA,[], 'nspk');
end

function RebuildPlots(src, event, plottype, varargin)

F = GetFigure(src);
%N.B. not GetDataFromFig - may have a parent figure, like combine
DATA = get(F,'UserData');
Expts = getappdata(DATA.allfig,'Expts');
for j = 1:length(Expts)
    if strcmp(plottype,'ContrastDynamics')
        Expts{j}.plotres = PlotRevCorAny(Expts{j},'nmin',2,'exp','cL','exp2','cR','box','sdfw',500,'noextra',varargin{:});
    else
        Expts{j}.plotres = PlotExpt(Expts{j},'rcnmin',5,'fbox',varargin{:});
    end
    drawnow;
end
setappdata(DATA.allfig,'Expts',Expts);
PlotAllCells(DATA,[], 'rates');
...
function PlotOneCell(a,b, plottype)
%set what kind of plot is used for one cell 
   if ishandle(a)
       DATA = get(GetFigure(a),'UserData');
   else
       DATA = a;
   end
   DATA.plot.onetype = plottype;    
   set(DATA.allfig,'UserData',DATA);
...
%       PlotRateSequences(DATA,plottype);
       return;
   elseif strncmp(plottype,'nspk',4)
       PlotRateSequences(DATA,plottype);
       DATA.plottype = plottype;
       SetPlotData(DATA, DATA.nexpts);
       set(DATA.allfig,'UserData',DATA);
       return;
   ...
   elseif strncmp(plottype,'load',4)
       ...
       setappdata(DATA.allfig,'AllCellPlot',AllCellPlot);
       setappdata(DATA.allfig,'AllCellRes',AllCellRes);
       return;
   end
   if isappdata(DATA.allfig,'Expts')
   ...
   for j = 1:length(Expts)
       ...
       if ~isfield(Expts{j},'plotres')
           Expts{j}.plotres = PlotExpt(Expts{j});
       end
...
%           P(1).Header.blocks = [Expts{j}.Trials(id).id];
           ...
           elseif isfield(P,'bestdelay')
               ...
               if yi > size(P.y,2)
                   fprintf('Too Few Rows for Cell %d\n',GetCellNumber(Expts{j}));
                   h = [];
               elseif strmatch(plottype,{'var' 'varim'})
           ...
           elseif isfield(P(1),'means')
               ...
               if size(P(1).n,2) >= yi %some cells might be missing some values
                   ...
                   if isfield(Expts{j}.Header,'cellnumber')
                       ...
                       labels{j} = sprintf('P%d',Expts{j}.Header.probe);
                   end
   ...
   SetPlotData(DATA, length(AllCellRes));
...
if DATA.toplevel > 0
    ...
    figpos.tag{1} = tag;
     ...
     if ~isempty(id) && (isnew || figpos(id).set ==0)
             ...
             figpos(id).set = 1;
%This should set the app data for a parent fig, if thats what toplevel is             
     ...
     if isnew && strcmp(DATA.tag.allexpts,tag)
         ...
         uimenu(sm,'label','Rate &Seq','Callback',{@PlotAllCells, 'rateseq'});
         uimenu(sm,'label','Rates','Callback',{@PlotAllCells, 'rates'});
         uimenu(sm,'label','Normalized (Max) Rates','Callback',{@PlotAllCells, 'normmax'});
         uimenu(sm,'label','Normalized (Max) Rate image','Callback',{@PlotAllCells, 'imagemax'});
         uimenu(sm,'label','Normalized (Mean) Rates','Callback',{@PlotAllCells, 'normmean'});
         uimenu(sm,'label','Normalized (Mean) Rate image','Callback',{@PlotAllCells, 'imagemean'});
         uimenu(sm,'label','Norm Rate &Seq','Callback',{@PlotAllCells, 'normrateseq'});
         uimenu(sm,'label','Block Rates','Callback',{@PlotAllCells, 'nspk'});
         uimenu(sm,'label','Rate &Seq+offset','Callback',{@PlotAllCells, 'offsetrateseq'});
         uimenu(sm,'label','Test','Callback',{@PlotAllCells, 'test'});
         sm = uimenu(hm,'label','&OneType');
         uimenu(sm,'label','&Mean Rate','Callback',{@PlotOneCell, 'meanresp'});
         uimenu(sm,'label','&AC loop','Callback',{@PlotOneCell, 'acloop'});
         uimenu(sm,'label','&Contrast Change','Callback',{@PlotOneCell, 'ContrastDynamics'});
         uimenu(sm,'label','Default','Callback',{@PlotOneCell, 'default'});
         sm = uimenu(hm,'label','&Rebuild');
         uimenu(sm,'label','&Default','Callback',{@RebuildPlots, 'defaults'});
         uimenu(sm,'label','&Contrast Dynamics','Callback',{@RebuildPlots, 'ContrastDynamics'});
         ...
         hm = uimenu(F,'label','&Options','Tag','PlotMenu');
         uimenu(hm,'label','Show Isolation Distnance','Callback',{@PlotAllCells, 'showisolation'});
...
function SetCellChooser(F)
    ...
    if length(AllCellRes) < length(Expts)
        for j = 1:length(Expts)
            AllCellRes(j).cellid = GetCellNumber(Expts{j});
            AllCellRes(j).Header.probe = GetProbeNumber(Expts{j});
        end
    end
...
function P =  NextPlot(P, AllCellRes, step)
    ...
    hold off;
    xstr = '';
    if strcmp(DATA.plottype,'rateseq')
        Expts = getappdata(DATA.toplevel,'Expts');
        E = Expts{P.currentcell};
        PlotRateSequence(E);
        title(sprintf('Cell %d (P%.1f)',E.Header.cellnumber,E.Header.probe));
        t = get(gca,'title');
    elseif strcmp(DATA.plot.onetype,'ContrastDynamics')
        R = PlotResult(AllCellRes(id(P.currentcell)),'plottype','novar','subres',0);
        t = get(gca,'title');
        xstr = '';
        ...
        R = PlotResult(AllCellRes(id(P.currentcell)),'plottype',DATA.plot.onetype);
        t = get(gca,'title');
        xstr = '';
        if isfield(R(1).Header,'probe')
            xstr = sprintf('P%d',R(1).Header.probe);
    ...
    if DATA.show.isolation
        Expts = getappdata(F,'Expts');
        E = Expts{P.currentcell};
        iso = E.Header.dips(:,3);
        xstr = sprintf('%s Iso %.1f+-%.1f',xstr,mean(iso),std(iso));
    end
...
function AllCellPlots(a,b, op, varargin)
     ...
     elseif isfield(DATA,'AllExpts')
         ename = Expt2Name(DATA.AllExpts{1});
     ...
     P =  getappdata(F,'AllCellPlot');
     if isnumeric(op)
         if length(AllCellRes) < op || isempty(AllCellRes(op))
             res = PlotExpt(Expts{op},'fbox','rcnmin',2);
             if isempty(AllCellRes)
                 clear AllCellRes;
                 AllCellRes(op) = res;                 
             else
                 AllCellRes = CopySFields(AllCellRes,op, res);
             end
             setappdata(F,'AllCellRes',AllCellRes);
             P.currentcell = op;
             figure(F);
     ...
     elseif strcmp(op,'xcorr')
         ...
         AllExpts = getappdata(F,'Expts');
         F = GetFigure('AllCell Xcorr');
         set(F,'Name','Calculating Xcorrs');
         drawnow;
         xc = ExptListCorrs(AllExpts);
         ExptPlotXcorr(xc,1);
         set(F,'Name','AllCell Xcorr');
         drawnow;
     ...
     setappdata(DATA.allfig,'AllCellPlot',P);
