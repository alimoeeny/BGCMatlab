...
%allows additions
%PlotComments(D, 'addhidden', text)  adds comment text to the file, with no
%GUI
%                ...,'noninteractive') returns list of comments, no GUI
%Uses two files  ExptComments.mat, which is generated from the .smr files
%and Comments.mat which are manually added comments
%
%see also BuildExptComments
parent = 0;
...
if ischar(varargin{1})
    if isdir(varargin{1})
        path = varargin{1};
    elseif exist(varargin{1}) && ~isempty(regexp(varargin{1},'pen.*\.log'))
        PlotOnePen(varargin{1},'comments');
        return;
    else
        [path, filename] = fileparts(varargin{1});
    end
    if ~exist(path)
        fprintf('No data folder %s\n',path);
        return;
    end
...
while j <= length(varargin)
    ...
    elseif strncmpi(varargin{j},'loadonly',5)
        return;
    elseif sum(strncmpi(varargin{j},{'noninteractive' 'nogui'},5))
        D.interactive = 0;
        if nargout == 0
            strargs = {strargs{:} '-show'};
        end
...
%        D.closeonadd = 0;
    ...
    elseif D.interactive
        ...
        if ~isempty(D.ExptComments)
            xf = setdiff(fields(D.ExptComments),{'text'});
            for j = 1:length(D.ExptComments)
                X(j).s = D.ExptComments(j).text;
                X(j).program = 'online';
                X(j).exptno = 0;
                for k = 1:length(xf);
                    X(j).(xf{k}) = D.ExptComments(j).(xf{k});
                end
            end
        else
            X = [];
        ...
        xf = setdiff(fields(D.Comments),{'comment'});
        for j = 1:length(D.Comments)
            X(end+1).s = D.Comments(j).comment;
            for k = 1:length(xf);
                X(end).(xf{k}) = D.Comments(j).(xf{k});
            end
        end
        end
        D = X;
        if sum(strcmp('-show',strargs))
            for j = 1:length(X)
                xs = '';
                if isfield(X,'exptno')
                    xs = sprintf('%s E%s',xs,sprintf('%d ',X(j).exptno));
                end
                if isfield(X,'user')
                    xs = sprintf('%s (%s)',xs,X(j).user);
                end
                if isfield(X,'time')
                    xs = sprintf('%s %s',xs,datestr(X(j).time));
                end
                fprintf('%s%s\n',X(j).s,xs);
            end
        end
    ...
    if D.interactive 
        ...
        if isnew
            ...
            uimenu(hm,'Label','Expt Lines','tag','exptlines','Callback',@SetOptions);
...
while j <= length(varargin)
    ...
    for j = 1:length(D.ExptComments)
        ...
        if D.show.user && isfield(D.ExptComments,'user')
            epstr = [epstr ' ' sprintf('ui=%s',D.ExptComments(j).user)];
        end
        if D.show.time            
