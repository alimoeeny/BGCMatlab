function [good, res, Expts, errdata] = CheckExpts(Expts, varargin)
%[good, res, Expts, errdata] = CheckExpts(Expts, varargin)
...
%res has error message
%Expts has modifield Expts with errors recorded
%errdata has error messages, used to build res
plotdelays = 0;
j = 1;
...
if iscell(Expts)
    ...
    for j = 1:length(Expts)
        ...
        if isfield(res{j},'errs')
            errdata = AddError(errdata, res{j});
        end
    end
...
if checkmode == 1 && isfield(Expt.Trials,'delay')
    ...
    if diff(minmax(durs)) > fz %> 1 frame scatter
        ...
        xid = find(abs(durs-d)>fz/2);
        firstt = Expt.Trials(xid(1)).Start(1)./10000;
        Expt = AddError(Expt,'-silent','Expt%d Duration varies: %d/%d outliers, %d at %.1f',GetExptNumber(Expt),b,length(id),xid(1),firstt);
    end
...
if isfield(Expt,'errs')
    res.errs = Expt.errs;
    if isfield(Expt,'errdata')
        res.errdata = Expt.errdata;
    end
end
