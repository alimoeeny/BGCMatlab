...
if ~isempty(eid)
    ...
    for j = 1:length(eid)
        ...
        if exist(name,'file')
            ...
            for t = 1:length(Expt.Trials)
                id = find(LFP.t > Expt.Trials(t).Start(1)-preperiod & ...
                    LFP.t < Expt.Trials(t).End(end)+postperiod);
                if ~isempty(id)
                    offset = round((LFP.t(id(1))- (Expt.Trials(t).Start(1)-preperiod))./(LFP.samper .*10000));
                    id = id-offset;
                    if id(1) > 1
                        gap(t) = LFP.t(id(1))-LFP.t(id(1)-1);
                    elseif id(1) <=0
                        gap(t) = -sum(id <=0);
                        id = id(id>0);
                    else
                    ...
                    Expt.Trials(t).lfpstart = LFP.t(id(1))-Expt.Trials(j).Start(1);
                    Expt.Trials(t).lfptime = LFP.t(id(1));
                    Expt.Trials(t).lfplen = length(id);
                    Expt.Trials(t).LFP = LFP.rawlfp(id,:);
                    if needft
                        Expt.Trials(t).FTlfp = abs(fft(Expt.Trials(t).LFP));
                    end
                    ng = ng+1;
                else %thie just measnt his rial not in current LFP File
                    nbad = nbad+1;
                end
            end
            id = find([Expt.Trials.lfplen] ==0);
            if verbose
    ...
    id = find([Expt.Trials.lfplen] ==0);
    if ~isempty(id)
        fprintf('Missing LFP Data for %d trials\n',length(id));
    end
end
