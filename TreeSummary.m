...
if ischar(path)
    ...
    elseif exist(path,'file') %read a text file made with find/ls-l
        [res, details] = ReadFileList(path);       
    end
...
function [res, details] = ReadFileList(name)
...
for j = 1:length(txt)
    s = split(txt{j});
    x(j).root = regexprep(s{9},'(./.*/).*','$1');
    x(j).size = str2num(s{5});
end

dirs = unique({x.root});
for j = 1:length(dirs)
    id = find(strcmp(dirs{j},{x.root}));
    res(j).root = dirs{j};
    res(j).files = length(id);
    res(j).bytes = sum([x(id).size]);
    fprintf('%s %d bytes in %d files\n',res(j).root,res(j).bytes,res(j).files);
end

fprintf('\n\nSorted:\n');
[a,b] = sort([res.bytes]);
for j = 1:length(b)
    fprintf('%s %d bytes in %d files\n',res(b(j)).root,res(b(j)).bytes,res(b(j)).files);
end
